...
$sql = "SELECT a.*, h.nom AS habitat_nom FROM animal a JOIN habitat h ON a.id_habitat = h.id_habitat";
$stmt = $conn->prepare($sql);
$stmt->execute();
...
foreach ($animals as $animal) {
    $animals_by_habitat[$animal['habitat_nom']][] = $animal;
}
?>
...
<main>
    ...
    <?php foreach ($animals_by_habitat as $habitat => $animals): ?>
        <section class="gest-animaux">
            <h2><?php echo htmlspecialchars($habitat); ?></h2>
                    ...
                    <?php if (!empty($animals)): ?>
                        <?php foreach ($animals as $index => $animal): ?>
                            <tr class="admin_tableau" style="background-color: <?php echo $index % 2 == 0 ? '#f2f2f2' : '#ffffff'; ?>">
                                ...
                                <td><?php echo htmlspecialchars($animal['etat_sante']); ?></td>
                                <td><?php echo htmlspecialchars($animal['type_alimentation']); ?></td>
                                <td>
                                    <a href="edit_animal.php?id=<?php echo $animal['id_animal']; ?>" class="edit-link" data-id="<?php echo $animal['id_animal']; ?>">Modifier</a>
                                    <a href="delete_animal.php" class="delete-link" data-id="<?php echo $animal['id_animal']; ?>">Supprimer</a>
                                </td>
                            </tr>
                        <?php endforeach; ?>
                    <?php else: ?>
                        <tr>
                            <td colspan="10">Aucun animal trouvé.</td>
                        </tr>
                    <?php endif; ?>
                </tbody>
            </table>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        confirmAction.addEventListener('click', function() {
            if (actionType === 'delete') {
                window.location.href = `delete_animal.php?id=${actionId}`;
            } else if (actionType === 'edit') {
                window.location.href = `edit_animal.php?id=${actionId}`;
            }
        });
